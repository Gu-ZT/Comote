...
function normalizeDingTalkConfig(config: JsonMap = {}) {
  return {
...
function normalizeDingTalkSecretPatch(config: JsonMap = {}) {
  const patch = { ...config };
  if (patch.appSecret === "" || patch.appSecret === "********") delete patch.appSecret;
  return patch;
}

function publicDingTalkConfig(config: JsonMap) {
  return {
...
function templatesFromConfig(config: JsonMap) {
  return {
...
function createDingTalkDriver(config: JsonMap) {
...
const dingtalkPlugin = {
  ...
  normalizeConfig: (raw: JsonMap) => normalizeDingTalkConfig(raw),
  normalizeSecretPatch: (raw: JsonMap) => normalizeDingTalkSecretPatch(raw),
  publicConfig: (config) => publicDingTalkConfig(config),
  createDriver: (config: JsonMap) => createDingTalkDriver(config),
  createRenderer: (config: JsonMap = {}) => createDingTalkRenderer({ templates: templatesFromConfig(config) }),
