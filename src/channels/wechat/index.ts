...
function normalizeWeChatConfig(config: JsonMap = {}) {
  return {
...
function publicWeChatConfig(config: JsonMap) {
  return {
...
function shouldStoreWeChatLoginResult(result: JsonMap) {
...
function createWeChatDriver(config: JsonMap) {
  if (!config.enabled) {
    return null;
  }
...
const wechatPlugin = {
  ...
  normalizeConfig: (raw: JsonMap) => normalizeWeChatConfig(raw),
  publicConfig: (config) => publicWeChatConfig(config),
  createDriver: (config: JsonMap) => createWeChatDriver(config),
