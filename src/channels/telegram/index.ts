...
function normalizeTelegramConfig(config: JsonMap = {}) {
  return {
...
function normalizeTelegramSecretPatch(config: JsonMap = {}) {
  const patch = { ...config };
  if (patch.botToken === "" || patch.botToken === "********") delete patch.botToken;
  return patch;
}

function publicTelegramConfig(config: JsonMap) {
  return {
...
function createTelegramDriver(config: JsonMap) {
...
const telegramPlugin = {
  ...
  normalizeConfig: (raw: JsonMap) => normalizeTelegramConfig(raw),
  normalizeSecretPatch: (raw: JsonMap) => normalizeTelegramSecretPatch(raw),
  publicConfig: (config) => publicTelegramConfig(config),
  createDriver: (config: JsonMap) => createTelegramDriver(config),
