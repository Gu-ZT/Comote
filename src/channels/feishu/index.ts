...
function normalizeFeishuConfig(config: JsonMap = {}) {
  return {
...
function normalizeFeishuSecretPatch(config: JsonMap = {}) {
  const patch = { ...config };
...
function publicFeishuConfig(config: JsonMap) {
  return {
...
function createFeishuDriver(config: JsonMap) {
...
function createFeishuLoginDriver({ domain = "feishu" }: { domain?: string } = {}) {
  return new FeishuDriver({
...
function shouldStoreFeishuLoginResult(result: JsonMap) {
...
const feishuPlugin = {
  ...
  normalizeConfig: (raw: JsonMap) => normalizeFeishuConfig(raw),
  normalizeSecretPatch: (raw: JsonMap) => normalizeFeishuSecretPatch(raw),
  publicConfig: (config) => publicFeishuConfig(config),
  createDriver: (config: JsonMap) => createFeishuDriver(config),
  createLoginDriver: ({ domain }: { domain?: string } = {}) => createFeishuLoginDriver({ domain }),
  shouldStoreLoginResult: (result) => shouldStoreFeishuLoginResult(result),
  normalizeLoginStatus: (raw: JsonMap = {}) => {
